<x-main-layout :title="$company->name" css="company_view">
    ...
    @if ($canEdit)
    <p><a href="{{ route('company.edit', $company) }}">Betrieb bearbeiten</a></p>
    @endif

    <p>{{ $company->description }}</p>
    @if ($company->sites->count() === 1 && $company->sites->first()?->location)
    <p>Standort: <a href="{{ route('location.view', $company->sites->first()->location) }}">{{ $company->sites->first()->location->name }}</a></p>
    @endif
    ...
    <section class="company-people" aria-labelledby="company-owners-heading">
        ...
        <ol>
            ...
            <li>
                <a href="{{ route('user.character', $owner->character) }}">{{ $owner->character?->name ?? 'Unbekannter Charakter' }}</a>
                @if (auth()->check() && (int) $owner->character?->user_id === (int) auth()->id() && $company->owners->count() > 1)
                <form class="company-inline-form" action="{{ route('company.owner.transfer', ['company' => $company, 'owner' => $owner]) }}" method="post">
                    @csrf
        ...
        @if ($canManageOwners)
        <form action="{{ route('company.owner.store', $company) }}" method="post">
            @csrf
    ...
    <details class="company-add-site">
        ...
        <form action="{{ route('company.site.store', $company) }}" method="post">
            @csrf
