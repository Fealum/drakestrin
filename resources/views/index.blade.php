<x-main-layout title="Sali Vuz!" css="index" :no-breadcrumbs="true">
    ...
    <div id="index-tiles">
        <div id="index-stats">
            <a href="{{ route('board.filter', ['filter' => 'date_last:1d']) }}"><em>{{ $postCount24h }}</em> Beiträge in den letzten 24 Stunden;</a>
            <a href="{{ route('board.filter', ['filter' => 'date_last:7d']) }}"><em>{{ $postCount7d }}</em> Beiträge in den letzten sieben Tagen.</a>
        </div>
        @if ($news)
        <div id="index-news">
        ...
        @if ($word)
        <div id="index-word">
            <a href="{{ route('dictionary.view', $word) }}">{{ $word->word }}</a>
