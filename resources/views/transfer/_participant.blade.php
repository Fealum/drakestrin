...
<a href="{{ route('user.character', $participant) }}"><x-avatar :subject="$participant" size="list" :title="$participant->name" /></a>
@elseif ($participant instanceof \App\Models\Territory\Location)
<a class="transfer-location" href="{{ route('location.view', $participant) }}">{{ $participant->name }}</a>
